...
export function StreakSection() {
  ...
  return (
    <section
      ref={sectionRef}
      className="relative w-full min-h-screen md:h-screen bg-brutal-bg overflow-hidden z-20 py-8 md:py-0"
    >
      ...
      <div className="md:hidden px-4 space-y-4">
        <div className="brutal-panel p-6">
          ...
          <div className="mt-4 flex items-baseline gap-3">
            <span className="text-5xl font-heading font-bold text-brutal-text">{currentStreak}</span>
            <span className="text-xl text-brutal-text-secondary font-heading">days</span>
          </div>
          <p className="text-sm text-brutal-text-secondary mt-4">
        ...
        <div className="brutal-panel p-6">
          ...
          <div className="mt-4 flex items-center gap-2 flex-wrap">
            {weekStatus.map((d, i) => (
              <div
                key={i}
                className={`w-8 h-8 rounded-full flex items-center justify-center font-mono text-xs font-medium transition-all ${
                  d.filled
          ...
          <div className="mt-4 flex items-baseline gap-2">
            <span className="text-2xl font-heading font-bold text-brutal-text">{attendedThisWeek}</span>
            <span className="text-brutal-text-secondary">/ 7</span>
          </div>
          <p className="text-sm text-brutal-text-secondary mt-2">
        ...
        <div className="brutal-panel p-6">
          <div className="grid grid-cols-3 gap-4">
            <div className="text-center">
              <span className="eyebrow block mb-2 text-xs">Total</span>
              <span className="text-2xl font-heading font-bold text-brutal-text">{getTotalCheckIns()}</span>
            </div>

            <div className="text-center border-x border-brutal-text/20">
              <span className="eyebrow block mb-2 text-xs">Best</span>
              <span className="text-2xl font-heading font-bold text-brutal-text">{longestStreak}</span>
            </div>

            <div className="text-center">
              <span className="eyebrow block mb-2 text-xs">Last</span>
              <span className="text-lg font-heading font-bold text-brutal-text">{getLastCheckIn()}</span>
            </div>
          </div>
        </div>
      </div>
